...
package com.byd.dct.iddm.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.byd.dct.iddm.common.xss.config.IddmXssProperties;
import com.byd.dct.iddm.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final IddmXssProperties properties;

	public DefaultXssCleaner(IddmXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(IddmXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		IddmXssProperties.Mode mode = properties.getMode();
		if (IddmXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (IddmXssProperties.Mode.validate == mode) {
			// 校验
